...
var _expose = function(exports, _) {
    ...
    /**
     * Adapts a set of activities in activitystrea.ms format to a simpler view
    ...
    var _adaptActivity = function(me, activity) {
        // Move the relevant items to the top
        ...
        var previews = _generatePreviews(activity);

        // Construct the adapted activity
        return new ActivityViewModel(activity, summary, primaryThumbnail, previews);
    };
     ...
     * @param {Activity}            activity         The activity to represent
     * @param {ActivityViewSummary} summary          [description]
     * @param {[type]} primaryThumbnail [description]
     * @param {[type]} previews         [description]
     */
    var ActivityViewModel = function(activity, summary, primaryThumbnail, previews) {
        var that = {
    ...
    /**
     * [ActivityViewSummary description]
     * @param {[type]} i18nKey    [description]
    ...
    var _prepareActivity = function(activity) {
        ...
        if (activity['oae:activityType'] === 'content-comment' || activity['oae:activityType'] === 'discussion-message') {
            ...
            _.each(comments, function(comment) {
                comment.thumbnail = _generateThumbnail(comment.author);
            });
    ...
    var _generatePrimaryThumbnail = function(activity) {
        ...
        return _generateThumbnail(actor);
    };

    var _generateThumbnail = function(entity) {
        return {
            'entity': {
                'id': entity.id,
                'profilePath': entity['oae:profilePath'] || entity.profilePath,
                'thumbnailUrl': (entity.image && entity.image.url) ? entity.image.url : null,
                'resourceType': entity.objectType,
                'resourceSubType': entity['oae:resourceSubType'] || entity.resourceSubType,
                'visibility': entity['oae:visibility'],
                'displayName': entity.displayName,
                'tenant': entity['oae:tenant'] || entity.tenant
            }
        };
    };

    var _generatePreviews = function(activity) {
        var previews = [];
        ...
        */
        ...
        if (previewObj['oae:wideImage']) {
            previews.push(_generateThumbnail(previewObj));
        } else {
            var previewItems = (previewObj['oae:collection'] || [previewObj]);
            _.each(previewItems, function(item) {
                previews.push(_generateThumbnail(item));
            });
        }
