...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        $(document).on('oae.context.send.comments', function(ev, contextData) {
            ...
            $(document).on('push.oae.content.' + contextProfile.id, function(ev, message) {
                // Ignore comments we created ourselves
                if (message.user && message.user.id !== oae.data.me.id) {
                    if (message.name === 'content-comment') {
                        renderComment(null, message.data.message, true);
                    } else if (message.name === 'content-comment-delete') {
                        if (message.data.deleteType === 'soft') {
                            markCommentAsDeleted(message.data.message);
                        } else {
                            infinityScroll.removeItems(message.data.commentCreatedTimestamp);
                        }
                    }
                }
            });
