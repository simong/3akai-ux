...
define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        var setUpPushNotifications = function() {
            ...
            var wiggle = function() {
                var degrees = ['2','4','2','0','-2','-4','-2','0', '2','4','2','0','-2','-4','-2','0', '2','4','2','0','-2','-4','-2','0'];
                var timeout = null;
                var wiggle = function() {
                    if (degrees.length === 0) {
                        clearInterval(timeout);
                        $('#topnavigation-notification-count').css({
                    ...
                    var degree = degrees.shift();
                    $('#topnavigation-notification-count').css({
                        '-webkit-transform': 'rotate(' + degree + 'deg) scale(1.5, 1.5)',
                        '-moz-transform': 'rotate(' + degree + 'deg) scale(1.5, 1.5)',
                        '-o-transform': 'rotate(' + degree + 'deg) scale(1.5, 1.5)',
                        '-sand-transform': 'rotate(' + degree + 'deg) scale(1.5, 1.5)',
                        'transform': 'rotate(' + degree + 'deg) scale(1.5, 1.5)'
                    });
                };
                timeout = setInterval(wiggle, 40);
            };

            var _handleWebsocketNotification = function(ev, message) {
            ...
            oae.api.websockets.subscribe(oae.data.me.id, 'notification', oae.data.me.signature);

            // Listen for new notifications coming in via the Push API.
            $(document).on('push.oae.notifications.' + oae.data.me.id, _handleWebsocketNotification);
        };
